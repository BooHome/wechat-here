package club.ihere.common.api.config;

import club.ihere.common.api.response.GetJsApiTicketResponse;
import club.ihere.common.api.response.GetTokenResponse;
...
import club.ihere.handle.ApiConfigChangeHandle;
...
public final class ApiConfig extends Observable implements Serializable {
    ...
    private void initToken(final long refreshTime) {
        ...
        HttpClientUtil.get(url, null, new HttpClientUtil.ResponseCallback() {
            @Override
            public void onResponse(int resultCode, String resultJson) {
                if (HttpStatus.SC_OK == resultCode) {
                    ...
                    if (null == response.getAccessToken()) {
                        ...
                        throw new ParameterValidationException("微信公众号token获取出错，错误信息:" + response.getErrcode() + "," + response.getErrmsg());
                    }
            ...
            public void onResponse(int resultCode, String resultJson) {
                if (HttpStatus.SC_OK == resultCode) {
                    ...
                    if (StringUtil.isBlank(response.getTicket())) {
                        ...
                        throw new ParameterValidationException("微信公众号jsToken获取出错，错误信息:" + response.getErrcode() + "," + response.getErrmsg());
                    }
