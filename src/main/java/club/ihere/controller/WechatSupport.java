...
import club.ihere.common.util.message.WeChatBizMsgCrypt;
import club.ihere.exception.AesException;
import club.ihere.handle.EventHandle;
import club.ihere.handle.MessageHandle;
import club.ihere.common.message.BaseMsg;
import club.ihere.common.message.TextMsg;
import club.ihere.common.message.req.*;
import club.ihere.common.util.wechat.MessageUtil;
import club.ihere.common.util.wechat.SignUtil;
...
import static club.ihere.common.util.current.BeanUtil.isNull;
import static club.ihere.common.util.current.BeanUtil.nonNull;
import static club.ihere.common.util.current.CollectionUtil.isEmpty;
import static club.ihere.common.util.current.CollectionUtil.isNotEmpty;
import static club.ihere.common.util.current.StringUtil.isNotBlank;
...
public abstract class WechatSupport {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(WechatSupport.class);
    //充当锁
    ...
    public String processRequest(HttpServletRequest request) {
        ...
        if (nonNull(msg)) {
            ...
            if (StringUtil.isNotBlank(getAESKey())) {
                try {
                    WeChatBizMsgCrypt pc = new WeChatBizMsgCrypt(getToken(), getAESKey(), getAppId());
